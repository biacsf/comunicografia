...
public class SRUResponse {
	...
	*/
	...
	public SRUResponse(Document d) throws Exception {
		...
		if( !ZS_RESPO.equals(lname) && !ZS_EXPLAIN_RESPO.equals(lname))
			throw new Exception("Not a valid SRU root element: " + lname);
		 ...
		 */
		version_nl = Helper.matchNodes("version", r);
		numrec_nl = Helper.matchNodes("numberOfRecords", r);
		rec_nl = Helper.matchNodes("records/record", r);
		
		if( version_nl == null || numrec_nl == null || rec_nl == null )
			throw new Exception("This record does not appear to be an SRU record.");
	...
	public class Record {
		...
		public Record(Element e) {
			...
			NodeList l = Helper.matchNodes("recordData", e);
